...
#to biblo baje fajn za pospešitev, kako jih uporabim?
#pip install accelerate   # multi-GPU / mixed precision
#pip install safetensors  # faster weight loading
...
print("naložen model kinetics400: ", model)
...
# # Load ViViT model from Hugging Face
...
# # Load datasets
# pose_dataset = load_dataset("openmmlab/PoseTrack21")
# activity_dataset = load_dataset("ucf101", split="train[:10%]")
# face_dataset = load_dataset("wider_face", split="train[:10%]")


# def preprocess_video(video_frames):
#     """Preprocess video frames for ViViT model"""
#     processed = processor(images=video_frames, sampling_rate=2, return_tensors="pt")
#     return processed["pixel_values"]


# def collate_fn(batch):
#     """Custom collate function for DataLoader"""
#     videos = torch.stack([torch.tensor(item["video"]) for item in batch])
#     labels = torch.tensor([item["label"] for item in batch])
#     return {"pixel_values": videos, "labels": labels}


# # Create DataLoaders
# train_loader = DataLoader(pose_dataset, batch_size=4, collate_fn=collate_fn)
# val_loader = DataLoader(activity_dataset, batch_size=4, collate_fn=collate_fn)
